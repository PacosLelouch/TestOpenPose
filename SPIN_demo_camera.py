...
sys.path.append(dir_path + '/SPIN/SPIN')
os.environ['PATH']  = os.environ['PATH'] + ';' + dir_path + '/SPIN/SPIN;'
...
def bbox_from_openpose(openpose_file, rescale=1.2, detection_thresh=0.2):
    ...
    #print('valid_keypoints.shape:', valid_keypoints.shape)
...
def process_image_resize(img, input_res=224):
    normalize_img = Normalize(mean=constants.IMG_NORM_MEAN, std=constants.IMG_NORM_STD)
    img = cv2.resize(img, (input_res, input_res), interpolation=cv2.INTER_AREA)
    img = img.astype(np.float32) / 255.
    img = torch.from_numpy(img).permute(2,0,1)
    norm_img = normalize_img(img.clone())[None]
    return img, norm_img
...
if __name__ == '__main__':
    ...
    #if cam_path != "":
    #    cap.open(cam_path)
    image_width = constants.IMG_RES
    image_height = constants.IMG_RES
    ...
    viewport_size = (int(viewport_size[0] * image_ratio), int(viewport_size[1] * image_ratio))
    ...
    renderer = MyRenderer(faces=smpl.faces, 
                          cfg={
                                  'FOCAL_LENGTH': constants.FOCAL_LENGTH,
                                  'IMAGE_SIZE': constants.IMG_RES,
                                  'IMAGE_STD': constants.IMG_NORM_STD,
                                  'IMAGE_MEAN': constants.IMG_NORM_MEAN,
                                  }, 
                          viewport_size=viewport_size)

    # Preprocess input image and generate predictions
    #img = cv2.imread(args.img)[:,:,::-1].copy() # PyTorch does not support negative stride at the moment
    
    record_time = 0.0
    named_window_name = 'Demo Camera SPIN'
    ...
    while cap.isOpened():
        ...
        img, norm_img = process_image_resize(img, input_res=constants.IMG_RES)#process_image(img, args.bbox, args.openpose, input_res=constants.IMG_RES)
        with torch.no_grad():
        ...
        cv2.imshow(named_window_name, img_shape)
...
#    
#        print('outfile = "%s"' % outfile, img_shape.shape, img_shape_side.shape)#TEST
#    
